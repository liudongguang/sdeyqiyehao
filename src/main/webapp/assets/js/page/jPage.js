    ...
    $.fn.page = function (options) {
        ...
        function PageDraw(json) {
            if (parseInt(settings.count) > 0) {
                var html = '<div><ul class="pagination  pagination-sm">';
                ...
                if (settings.skipPart == true) {
                    $(that).append('<div><span class="text-muted align-middle" style="margin:5px;margin-left:10px;display:inline-block;font-size:14px">共有' + pageCount + '页/' + settings.count + '条 <input type="text" id="toPageNumID" class="form-control d-inline"/>页<button class="btn btn-outline-primary btn-sm" id="pageRealBTID">确定</button></span></div>');
                }
            } else {
